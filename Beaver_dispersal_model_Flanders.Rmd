...
data_first <- data_rds %>% filter(run == 0 & jaar == 0) %>% 
...
points_mod <- data_rds %>%
  st_transform(crs = 4326)
tmp <- st_join(punten_mod, UTM1, join = st_intersects) %>%
  group_by(TAG) %>%
  summarise(aantal = length(TAG)) %>%
  st_drop_geometry()

Shape_mod_dummy <- merge(UTM1, tmp, by='TAG', all.x=T) %>%
  mutate(aantal = replace_na(aantal,0))

Shape_mod_dummy$Bezettingskans <- cut(Shape_mod_dummy$aantal,
                                      breaks = c(0,1,5,20,Inf),
                                      labels = c("Accidenteel","Laag",
                                                 "Middel","Hoog"), 
                                      include_lowest = TRUE, 
                                      right = FALSE)

factpal <- colorFactor(heat.colors(4), Shape_mod_dummy$Bezettingskans, 
                       reverse=T)

kaart <- leaflet(subset(Shape_mod_dummy, aantal>0)) %>%
  addTiles() %>%
  addPolygons(color = ~factpal(Bezettingskans),
              opacity=0,
              fillOpacity=0.8) %>%
  addLegend(pal = factpal, values = ~Bezettingskans)

kaart

saveWidget(kaart, file = "kaart_bezettingskans_2025_2027.html", selfcontained = TRUE)

barplot(table(Shape_mod_dummy$Bezettingskans))
