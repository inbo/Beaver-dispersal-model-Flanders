...
This code describes a model that simulates dispersal movements of beavers in Flanders. As starting points for dispersal we used a layer with 457 known territories of beavers in Flanders, represented by point locations.  As a habit suitability layern the raster output of a Maxent model (projection_wide) for Flanders and surrounding 10 km buffer is used. This raster layer is combined with a layer containing the major river systems in this area as a linestring object.We also use an UTM 1*1 km square layer of Flanders to project the output of this dispersal model on. 

This code models how beavers disperse in Flanders. It starts with a layer of 457 known beaver territories, represented as points. 

The model uses a few key layers to simulate beaver movement:

* Habitat Suitability: A raster layer created by a Maxent model (called "projection_wide") that shows suitable beaver habitats in Flanders and a 10 km buffer around it.

* River Systems: A linestring layer representing the major rivers in the same area. This is combined with the habitat suitability layer.

* Grid: An UTM 1x1 km square layer of Flanders. This layer is used to project the final output of the dispersal model, making it easier to visualize and analyze the results.
...
In the next code chunk, different functions needed within the model itself are defined:
* the levy walker function

* the simulate_row function: this function allows 3 levy walker simulations per step, stores valid outcomes and alters suitability accordingly

* the randomRows function: this function samples the number of points needed to mimic a growing population
...
This code selects the number of starting points (preferably all, limited for the demo) and defines the number of model runs (z). It stores the outcome of each model run as an .RDS file.
...
These two code chunks read all .RDS files, combine them and map them onto the 1*1 km square grid.
